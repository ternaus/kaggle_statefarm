...
if __name__ == '__main__':
	...
	model_weights = 'model_weightsVGG_32_1_.h5'
	model_architecture = 'architectureVGG_32_1_.json'
	...
	X_test_id = np.array(f['X_test_id'])
	...
	model = model_from_json(open(os.path.join(cache_path, model_architecture)).read())
	model.load_weights(os.path.join(cache_path, model_weights))
	...
	result.loc[:, 'img'] = pd.Series(X_test_id, index=result.index)
